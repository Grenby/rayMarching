...
float SDF_world(vec3 p){
    ...
    int iter = 4;
    float zz = opUnion(sdf_fractal1(p,sdf_torusXY(p,torus_r0),iter),sdf_fractal1(p,sdf_box(p,vec3(3,3,3)),iter));
    zz = opUnion(zz,sdf_fractal1(p,sdf_torusYZ(p,torus_r0),iter));
    zz = opUnion(zz,sdf_fractal1(p,sdf_torusZX(p,torus_r0),iter));
